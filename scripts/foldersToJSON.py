        ...
        if os.path.isdir(auditor_path):
            for auditee in os.listdir(auditor_path):
                ...
                if os.path.isdir(auditee_path):
                    for date_dir in os.listdir(auditee_path):
                        date_path = os.path.join(auditee_path, date_dir)
                        if os.path.isdir(date_path):
                            for report in os.listdir(date_path):
                                if report.endswith('.pdf'):
                                    report_path = f"https://governance.felixschuler.at/reports/{auditor}/{auditee}/{date_dir}/{report}"
